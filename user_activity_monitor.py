...
def signup_window():
    ...
    signup_win.geometry("500x300")  # Increased window height for better visibility
    signup_win.configure(bg="#D3D3E7")  # Set background color

    style = ttk.Style(signup_win)
    style.theme_use("alt")

    # Customize styles
    style.configure("TLabel", background="#1F1F2E", foreground="#D3D3E7", font=("Arial", 12))
    style.configure("TButton", background="#3A3A4F", foreground="#FFFFFF", font=("Arial", 10, "bold"))
    style.map("TButton", background=[("active", "#6A4F4B")])
    style.configure("Hover.TButton", background="#6A4F4B", foreground="#FFFFFF", font=("Arial", 10, "bold"))
    ...
    entry_confirm_password = ttk.Entry(signup_win, show="*", font=("Helevicta", 12))
...
def login_window():
    ...
    login_win.geometry("400x400")  # size of login page
    login_win.configure(bg="#D3D3E7") # background colour change hoga

    style = ttk.Style(login_win)
    style.theme_use("clam") #change login and signup button
    

    # Customize styles
    style.configure("TLabel", font=("Arial", 12)) # username and password colour
    style.configure("TButton", background="#FF4081", foreground="#ECEFF4", font=("Arial", 10, "bold")) # logina dn soignup button color change
...
def initialize_gui(username):
    ...
    root.configure(bg="#000000") # color change krna hai login krke background

    style = ttk.Style()
    style.theme_use("clam")
    style.configure("Treeview", background="#3B4252", foreground="white", rowheight=25, fieldbackground="#3B4252") #change the color of bar
    style.configure("Treeview.Heading", background="#27c416", foreground="#ECEFF4", font=("Helvetica", 10, "bold")) # change the color of key press mouse clicks
